...
void AItem::FinishInterping()
{
	...
	if (Character)
	{
		Character->IncermentInterpLocItemCount(InterpLocIndex, -1);
...
void AItem::ItemInterp(float DeltaTime)
{
	...
	if (Character && ItemZCurve)
	{
		...
		// vector from item to camera interp lactaion x and y zeroed out
		const FVector ItemToCamera{ FVector(0.f, 0.f, (CameraInterpLocation - ItemLocation).Z) };
		const float DeltaZ = ItemToCamera.Z;

		const FVector CurrentLovation{ GetActorLocation() };
		const float InterpXValue = FMath::FInterpTo(CurrentLovation.X, CameraInterpLocation.X, DeltaTime, 30.0f);
		const float InterpYValue = FMath::FInterpTo(CurrentLovation.Y, CameraInterpLocation.Y, DeltaTime, 30.0f);
...
void AItem::OnConstruction(const FTransform& Transform)
{
	...
	{
		//UE_LOG(LogTemp, Warning, TEXT("RarityTableObject is Valided."));
		...
		if (RarityRow)
		{
			...
			MaxNormalDamageRate = RarityRow->MaxNormalDamageRate;
			MaxCriticalRate = RarityRow->MaxCriticalRate;
			CriticalRate = RarityRow->CriticalRate;
